...
const EMPLOYERS = {
 'Austin': [
 { name: 'Tesla Gigafactory', dist: '15 to 25 min E' },
 { name: 'Apple Campus (Domain)', dist: '20 to 30 min N' },
 { name: 'Google / Meta / Indeed', dist: '10 to 15 min downtown' },
 { name: 'Dell Medical School / UT Austin', dist: '10 to 20 min' },
 ...
 { name: 'Williamson County jobs hub', dist: '5 to 10 min' },
 ],
 'Pflugerville': [
 ...
 'Kyle': [
 { name: 'Hays County job center', dist: '5 to 10 min' },
...
function renderListingPage(listing, { market, comps, neighborhood, employers, investment } = {}) {
 ...
 numberOfRooms: listing.bedrooms_total, numberOfBathroomsTotal: listing.bathrooms_total...(listing.living_area ? { floorSize: { '@type': 'QuantitativeValue', value: listing.living_area, unitCode: 'FTK' } } : {})...(listing.year_built ? { yearBuilt: listing.year_built } : {})...(photoUrls.length ? { image: photoUrls.slice(0, 6) } : {})...(listing.latitude && listing.longitude ? { geo: { '@type': 'GeoCoordinates', latitude: listing.latitude, longitude: listing.longitude } } : {}),
 ...
 ['MLS #', listing.listing_key||', '], ['List Date', fmtDate(listing.listing_contract_date)||', ']...(isSold ? [['Close Date', fmtDate(listing.close_date)||', '],['Close Price', fmtPrice(listing.close_price)||', ']] : []),
