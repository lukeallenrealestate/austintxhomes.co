...
// Hyde Park is one of Austin's oldest neighborhoods (platted 1890s as Austin's
// first planned subdivision by Monroe Shipe). It's geographically defined by
// ZIP 78751, bounded roughly by Guadalupe St (west), Airport Blvd (east),
// 38th St (south), and 51st St (north). The dominant elementary schools are
// Lee Elementary, Ridgetop, and Reilly. Subdivision-name filtering misses too
// much (only catches "Hyde Park Add 01/02/Annex" labels) so we use ZIP 78751.
console.log('Loading residential records (Hyde Park / 78751)...');
const ALL = q(`SELECT * FROM listings WHERE postal_code = '78751' AND property_type = 'Residential' AND property_sub_type IN ('Single Family Residence','Condominium','Townhouse')`);
...
console.log('═══════════════════════════════════════════════════════════════');
console.log(' HYDE PARK 2026 DEEP MARKET ANALYSIS');
console.log('═══════════════════════════════════════════════════════════════');
console.log();
...
const ageBuckets = {
 '<1930 (Original Hyde Park)': ages.filter(y => y < 1930).length,
