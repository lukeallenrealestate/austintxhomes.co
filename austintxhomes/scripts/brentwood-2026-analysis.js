#!/usr/bin/env node
/**
 * Brentwood 2026 Market Report — Deep Data Extraction
 *
 ...
 *
 * Filter rationale: Brentwood is part of ZIP 78757, which also covers
 * Bouldin Creek, Travis Heights, South Lamar, South Congress, and
 * parts of Barton Hills. ZIP alone is too broad. Subdivision_name
 * ("Brentwood") catches only a fraction. Brentwood Elementary School zone
 ...
 * returns within a small margin. Combined 78757 + Brentwood Elementary.
 */
...
console.log('Loading residential records (Brentwood ES zone in 78757) ...');
const ALL = q(`SELECT * FROM listings WHERE postal_code = '78757' AND elementary_school = 'Brentwood' AND property_type = 'Residential' AND property_sub_type IN ('Single Family Residence','Condominium','Townhouse')`);
...
function bucket(y) {
  if (!y) return 'Unknown';
  if (y < 1960) return 'Pre-1960 (Original Brentwood)';
...
// ─── CONDOS ────────────────────────────────────────────────────────
console.log('── CONDOS (Brentwood zone in 78757) ──');
