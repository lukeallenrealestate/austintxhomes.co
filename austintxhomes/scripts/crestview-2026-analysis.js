#!/usr/bin/env node
/**
 * Crestview 2026 Market Report — Deep Data Extraction
 *
 ...
 *
 * Filter rationale: Crestview is part of ZIP 78757, which also covers
 * Bouldin Creek, Travis Heights, South Lamar, South Congress, and
 * parts of Barton Hills. ZIP alone is too broad. Subdivision_name
 * ("Crestview") catches only a fraction. Crestview Elementary School zone
 ...
 * returns within a small margin. Combined 78757 + Crestview Elementary.
 */
...
console.log('Loading residential records (Crestview via subdivision_name LIKE) ...');
// Crestview shares Brentwood Elementary with the adjacent Brentwood neighborhood,
// so the school filter would double-count. The subdivision-name LIKE filter
// captures Crestview proper most accurately (~15 active vs polygon SSR's ~27).
const ALL = q(`SELECT * FROM listings WHERE subdivision_name LIKE '%Crestview%' COLLATE NOCASE AND property_type = 'Residential' AND property_sub_type IN ('Single Family Residence','Condominium','Townhouse')`);
...
function bucket(y) {
  if (!y) return 'Unknown';
  if (y < 1960) return 'Pre-1960 (Original Crestview)';
...
// ─── CONDOS ────────────────────────────────────────────────────────
console.log('── CONDOS (Crestview zone in 78757) ──');
