#!/usr/bin/env node
/**
 * Zilker 2026 Market Report — Deep Data Extraction
 *
 ...
 *
 * Filter rationale: Zilker is part of ZIP 78704, which also covers
 * Bouldin Creek, Travis Heights, South Lamar, South Congress, and
 * parts of Barton Hills. ZIP alone is too broad. Subdivision_name
 * ("Zilker") catches only a fraction. Zilker Elementary School zone
 ...
 * returns within a small margin. Combined 78704 + Zilker Elementary.
 */
...
console.log('Loading residential records (Zilker ES zone in 78704)...');
const ALL = q(`SELECT * FROM listings WHERE postal_code = '78704' AND elementary_school = 'Zilker' AND property_type = 'Residential' AND property_sub_type IN ('Single Family Residence','Condominium','Townhouse')`);
...
function bucket(y) {
  if (!y) return 'Unknown';
  if (y < 1960) return 'Pre-1960 (Original Zilker)';
...
// ─── CONDOS ────────────────────────────────────────────────────────
console.log('── CONDOS (Zilker zone in 78704) ──');
