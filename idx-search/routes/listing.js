...
router.get('/:slug', (req, res) => {
 ...
 url: canonicalUrl,
 description: public_remarks ? truncate(public_remarks, 300) : ''...(list_price && { price: String(list_price), priceCurrency: 'USD' }),
 address: {
 '@type': 'PostalAddress',
 ...
 })...(year_built && { yearBuilt: String(year_built) })...(living_area && { floorSize: { '@type': 'QuantitativeValue', value: living_area, unitCode: 'FTK' } })...(beds && { numberOfRooms: beds }),
...
<div id="seo-prerender" style="position:absolute;width:1px;height:1px;overflow:hidden;clip:rect(0,0,0,0);" aria-hidden="true">
 ...
 <p>${escHtml(price)}, ${escHtml(prerenderStats)}</p>
